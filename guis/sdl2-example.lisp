    ...
    (sdl2:with-window (win :flags '(:shown :opengl))
      (sdl2:with-gl-context (gl-context win)
        (let ((controllers ())
          ...
          (format-now "Setting up window/gl.~%")
          ...
          (sdl2:with-event-loop (:method :poll)
            (:keydown (:keysym keysym)
             (let ((scancode (sdl2:scancode-value keysym))
               ...
               (cond
                 ((sdl2:scancode= scancode :scancode-w) (format-now "~a~%" "WALK"))
                 ((sdl2:scancode= scancode :scancode-s) (sdl2:show-cursor))
                 ((sdl2:scancode= scancode :scancode-h) (sdl2:hide-cursor)))
               (format-now "Key sym: ~a, code: ~a, mod: ~a~%"
                       sym
                       scancode
                       mod-value)))
            ...
            (:mousemotion (:x x :y y :xrel xrel :yrel yrel :state state)
             (format-now "Mouse motion abs(rel): ~a (~a), ~a (~a)~%Mouse state: ~a~%"
                     x xrel y yrel state))

            (:controlleraxismotion (:which controller-id :axis axis-id :value value)
             (format-now "Controller axis motion: Controller: ~a, Axis: ~a, Value: ~a~%"
          ...
          (format-now "Closing opened game controllers.~%")
